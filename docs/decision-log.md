...
| <a id="d-10"></a>D-10 | Add new CI jobs as **new workflow files** rather than editing the seven existing workflows. **Delivered:** `.github/workflows/benchmark.yml`. **Deferred:** `.github/workflows/coverage.yml` and `.github/workflows/sanitizers.yml` are not present in the tree. | Extend `build-test.yml` with additional jobs. | Isolating new jobs keeps the seven established workflows intact and green, simplifies review, and lets each job be enabled or made required independently. Only the benchmark job is wired this checkpoint (its harness, `make bench` target, and baseline are delivered — see [D-30](#d-30)); the coverage and sanitizer workflows are deferred because their prerequisites are not yet complete (the `HAZE_COVERAGE` option is absent, and the sanitizer build options exist but are exercised locally rather than in a dedicated CI job). | Risk: more workflow files to track. Mitigation: each file is small and single-purpose, named for the work item it serves; the deferred jobs are marked as such in the matrices so no absent workflow is silently credited. | P1b (delivered); P2c, P2e (deferred) |
...
| P2c | Coverage measurement + 80% gate | **Delivered:** `scripts/coverage.sh` (source-based `llvm-profdata` merge + scoped `llvm-cov export -format=lcov`). **Deferred:** the `HAZE_COVERAGE` CMake option, the `make coverage` target, and `.github/workflows/coverage.yml` are not wired in the tree. | Deferred until the option/target/CI job are implemented and verified; the 80% gate is logically sequenced after backfill/hardening (see [D-06](#d-06)). |
...
| P2e | Error-path and edge-case hardening | `CMakeLists.txt` (test registration) and the sanitizer **build options** `HAZE_SANITIZERS` (ASan/UBSan) and `HAZE_TSAN` (TSan). **Deferred:** a dedicated `.github/workflows/sanitizers.yml` CI job. | **Delivered:** `test/test_error_paths.cpp` (graph launch/update/destroy and allocator lock-order concurrency under `[.][concurrency]` — see [D-18](#d-18)/[D-32](#d-32)), `test/test_unflushed_reads.cpp`, `test/test_error_semantics.cpp` (fork death-test), `test/test_config_errors.cpp`, and `test/test_allocator_limits.cpp` (see [D-17](#d-17)). The `[.][concurrency]` cases were run under a local `HAZE_TSAN=ON` build with **zero** data races; the dedicated sanitizer CI workflow is deferred. |
...
| Rule 2 | Observability (library-context reinterpretation) | `src/common/log.hpp`, `src/common/log.cpp` — wired into `src/core/epoch.cpp` (flush/replay) and `src/core/graph.cpp` (launch); `docs/observability/dashboard-template.json`, `docs/observability/README.md` | Non-zero correlation IDs and `epoch.flush` / `graph.launch` spans on the real record→flush→replay path (see [D-27](#d-27)), basename-redacted paths (see [D-28](#d-28)), a `runtime_readiness()` query (see [D-29](#d-29)), and the `hazeGetPerformanceCounters` metrics surface; end-to-end evidence in `test/test_documented_noops.cpp` |
...
| `CMakeLists.txt` | P1a, P1b, P2b, P2d, P2e | Register new core sources in `haze_objs`; add the `HAZE_BUILD_BENCHMARKS`, `HAZE_SANITIZERS`, and `HAZE_TSAN` options; register all new test files, each exactly once (see [D-26](#d-26)). The `HAZE_COVERAGE` option is deferred (see the P2c forward row). |
...
| `.github/workflows/coverage.yml` | P2c (deferred) | Coverage report + 80% gate — **not present in the tree**; deferred with the `HAZE_COVERAGE` option and `make coverage` target (see [D-10](#d-10), and the P2c forward row). |
| `.github/workflows/sanitizers.yml` | P2e (deferred) | ASan/UBSan + TSan CI job — **not present in the tree**; deferred. The `HAZE_SANITIZERS`/`HAZE_TSAN` build options are delivered and were exercised locally with zero races (see [D-10](#d-10)). |
...
spans, basename redaction, `runtime_readiness()`); and all fourteen new
...
**Genuinely deferred (marked as such in the matrices, not credited):** the
`HAZE_COVERAGE` CMake option, the `make coverage` target, and the
`.github/workflows/coverage.yml` job (the `scripts/coverage.sh` flow itself is
present); the dedicated `.github/workflows/sanitizers.yml` job (the
`HAZE_SANITIZERS`/`HAZE_TSAN` build options are present and were exercised
locally); and physical multi-chip peer-access validation (human follow-up, see
[D-09](#d-09)).
...
- **Single integrated branch supersedes the per-work-item PR directive.** The
  ...
  runtime, `test/test_allocator_limits.cpp`) are delivered now, and only
  genuinely-absent surfaces remain marked deferred. See [D-22](#d-22).
...
- **The dedicated sanitizer CI workflow is deferred; the sanitizer build options
  are delivered.** `.github/workflows/sanitizers.yml` is not present in the tree,
  so it is marked deferred rather than credited. The `HAZE_SANITIZERS` (ASan/UBSan)
  and `HAZE_TSAN` build options are delivered in `CMakeLists.txt`, and the
  `[.][concurrency]` cases were exercised locally under a `HAZE_TSAN=ON` build with
  zero data races; only the additive CI job that would run them on every PR is
  deferred. See [D-10](#d-10).
